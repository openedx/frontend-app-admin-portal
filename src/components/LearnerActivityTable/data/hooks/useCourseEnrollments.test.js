...
describe('useCourseEnrollments', () => {
  ...
  it('should fetch course enrollments successfully', async () => {
    ...
    EnterpriseDataApiService.fetchCourseEnrollments.mockResolvedValue(mockResponse);
  ...
  it('should handle errors when fetching course enrollments', async () => {
    ...
    EnterpriseDataApiService.fetchCourseEnrollments.mockRejectedValue(error);
  ...
  it('should calculate pageCount when numPages is not provided', async () => {
    ...
    expect(result.current.courseEnrollments.pageCount).toBe(2); // 25/10 = 2.5 -> Math.floor = 2
  });

  it('should ensure fetchCourseEnrollments is debounced', () => {
    const { result } = renderHook(() => useCourseEnrollments(enterpriseId, tableId, apiFieldsForColumnAccessor));

    // The returned function should be from debounce (has cancel and flush methods)
    expect(result.current.fetchCourseEnrollments).toHaveProperty('cancel');
    expect(result.current.fetchCourseEnrollments).toHaveProperty('flush');
  });
});
