...
const LearnerActivityTable = ({ id, enterpriseId, activity }) => {
  ...
  const pageFromUrl = parseInt(queryParams.get('page'), 10) || 1; // Default to page 1 in URL
  ...
  const fetchTableData = useCallback((tableState) => {
    ...
    if (newPageForUrl !== 1) { // Default page is 1
      newQueryParams.set('page', newPageForUrl);
    } else {
  ...
  }, [fetchCourseEnrollments, location.pathname, location.search, navigate]);
