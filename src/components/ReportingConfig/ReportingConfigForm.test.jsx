...
function testConfigForm(mock, config = defaultConfig) {
  return (
    <ReportingConfigForm
      config={config}
...
describe('<ReportingConfigForm />', () => {
  ...
  it('check the compression checkbox is render or not', () => {
    const mock = jest.fn();
    render(testConfigForm(mock));
    const checkbox = screen.getByRole('checkbox', { name: 'Enable compression' });

    expect(checkbox.checked).toBe(true);
    userEvent.click(checkbox);
    expect(checkbox.checked).toBe(false);
  });
  it('renders the proper fields with the sftp delivery method', async () => {
    const mock = jest.fn();

    const config = { ...defaultConfig };
    config.deliveryMethod = 'sftp';
    render(testConfigForm(mock, config));

    expect(screen.queryByText('SFTP hostname')).not.toBeNull();
    expect(screen.queryByText('SFTP port')).not.toBeNull();
    expect(screen.queryByText('SFTP username')).not.toBeNull();
    expect(screen.queryByText('Encrypted SFTP password')).not.toBeNull();
    expect(screen.queryByText('SFTP file path')).not.toBeNull();
  });
