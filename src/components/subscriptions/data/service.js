...
export function createSampleUser(licenseStatus) {
  return {
    ...
    pendingSince: moment(faker.date.past(10)).format('MMM DD, YYYY'),
    licenseStatus,
  };
}
...
export function sendLicenseReminder(options = {}) {
  ...
  const pendingSince = moment().format('MMM DD, YYYY');
  const updatedUser = updateUserRemindTimeStamp({ userId, bulkRemind, pendingSince });
  const response = {
