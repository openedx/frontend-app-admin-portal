...
import { REQUEST_STATUS_FILTER_CHOICES, REQUEST_TAB_VISIBLE_STATES, BNR_REQUEST_PAGE_SIZE } from '../constants';
...
const getRequestStatusDisplayName = (latestAction) => latestAction?.status?.toLowerCase().replace(/\s+/g, '_');
...
const transformApiDataToTableData = (apiResults) => apiResults.map((item) => {
  ...
  return {
    ...
    lastActionStatus: getRequestStatusDisplayName(item?.latestAction),
  ...
  const fetchBnrRequests = useCallback(async (args) => {
    ...
    try {
      ...
      const options = {
        ...
        pageSize: args.pageSize || BNR_REQUEST_PAGE_SIZE,
      ...
      setBnrRequests({
        ...
        pageCount: data.numPages || Math.ceil((data.count || 0) / options.pageSize),
