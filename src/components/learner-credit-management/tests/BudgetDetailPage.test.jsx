...
jest.mock('../data', () => ({
  ...
  useRemindContentAssignments: jest.fn(),
}));
...
describe('<BudgetDetailPage />', () => {
  ...
  //   });
  //   renderWithRouter(<BudgetDetailPageWrapper />);
  //   const remindRowAction = screen.getByTestId('datatable-select-column-checkbox-header');
  //   expect(remindRowAction).toBeInTheDocument();
  ...
  //   const remindDialogButton = screen.getByRole('button', { name: 'Send reminders (2)' });
  //   userEvent.click(remindDialogButton);
  //   expect(
  //     EnterpriseAccessApiService.remindAssignments,
  //   ).toHaveBeenCalled();
  //   await waitFor(
  //     () => act(() => expect(screen.getByText('Reminders sent (2)')).toBeInTheDocument()),
  //   );
  // });
  ...
  it('reminds a single assignment', async () => {
    ...
    const modalDialog = screen.getByRole('dialog');
    expect(modalDialog).toBeInTheDocument();
    const remindDialogButton = screen.getByRole('button', { name: 'Send reminder' });
    screen.debug(undefined, 10000000);
    userEvent.click(remindDialogButton);
    await waitFor(
      () => act(() => expect(screen.getByText('Reminder sent')).toBeInTheDocument()),
    );
  });
});
