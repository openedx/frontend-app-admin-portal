...
const getRequestStatusDisplayName = (requestStatus) => {
...
const BudgetDetailApprovedRequestTable = ({
  ...
  const statusFilterChoices = tableData.requestStatusCounts
    ? tableData.requestStatusCounts
      .filter(
        ({ requestStatus }) => !!getRequestStatusDisplayName(requestStatus),
      )
      .map(({ requestStatus, count }) => ({
        name: getRequestStatusDisplayName(requestStatus),
        number: count,
        value: requestStatus,
      }))
    : [];
