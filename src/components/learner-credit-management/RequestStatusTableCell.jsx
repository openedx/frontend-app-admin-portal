...
import GenericError from './request-status-chips/GenericError';
...
const RequestStatusTableCell = ({ enterpriseId, row }) => {
  ...
  const currentStatus = lastActionStatus || requestStatus;
  ...
  if (requestStatus !== 'Approved' && lastActionErrorReason) {
    return (
      <GenericError
        errorReason={lastActionErrorReason}
        trackEvent={sendGenericTrackEvent}
        recentAction={lastActionStatus}
      />
    );
  }
  ...
  if (currentStatus === 'waiting_for_learner') {
    return (
      <WaitingForLearner
        learnerEmail={learnerEmail}
        trackEvent={sendGenericTrackEvent}
  ...
  return <Chip>{`${capitalizeFirstLetter(currentStatus)}`}</Chip>;
};
